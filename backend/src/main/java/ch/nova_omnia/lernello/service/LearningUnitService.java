...
public class LearningUnitService {
    ...
    public LearningUnitService(LearningUnitRepository learningUnitRepository, LearningKitRepository learningKitRepository) {
    ...
    public Block createBlock(UUID learningUnitId, CreateBlockDTO createBlockDTO) {
        LearningUnit unit = learningUnitRepository.findById(learningUnitId).orElseThrow(() -> new RuntimeException("Learning Unit not found"));

        Block block = switch (createBlockDTO.type().toLowerCase()) {
            case "quiz" -> new QuestionBlock();
